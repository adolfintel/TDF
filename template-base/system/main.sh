...
function runSandboxed {
    ...
    for f in "/usr" "/bin" "/lib" "/lib32" "/lib64" "/sys" "/etc/hosts" "/etc/hostname" "/etc/localtime" "/etc/resolv.conf" "/etc/fonts" "/etc/machine-id" "/etc/alsa" "/etc/pipewire" "/etc/pulse" "/etc/ssl" "/etc/ca-certificates"; do
        if [ -e "$f" ]; then
...
function _applyFSR4 {
    if [ -e "system/fsr4" ]; then
        if [[ -n "$FSR4_UPGRADE" && "$FSR4_UPGRADE" -eq 1 ]]; then
